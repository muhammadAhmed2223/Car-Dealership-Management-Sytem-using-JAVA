...
public class AdminReader {
	public ObservableList<Admin> getAllAdmins(){
		...
		try {
			...
			while (result.next()) {
				list.add(new Admin(Integer.parseInt(result.getString(1)),result.getString(2), result.getString(3), result.getString(4), result.getString(5), result.getString(6), result.getString(7) ));
			}
		}
		catch (Exception e){
